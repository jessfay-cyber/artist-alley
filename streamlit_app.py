...
Full build: password, combo deals, goals with costs, series colors,
...
try:
    ...
    st.error("🚨 Unexpected error connecting to Google Sheets.")
    st.code(str(e))
    st.stop()
    ...
    try:
        ...
        for r in records:
            ...
            if not s:
                continue
            if r.get("color"):
                colors[s] = str(r["color"]).strip()
            if r.get("icon"):
                icons[s] = str(r["icon"]).strip()
        return colors, icons
    except Exception:
        return {}, {}
...
def calculate_combos(cart, inventory, deals):
    ...
    priority = {"bundle": 0, "qty_same": 1, "bogo": 2, "mix_category": 3, "cart_percent": 4}
    deals_sorted = sorted(deals, key=lambda d: priority.get(d.get("deal_type", ""), 99))
    ...
    for d in deals_sorted:
        ...
        if dtype == "qty_same":
            ...
            cat = d.get("trigger_category", "")
            eligible = [c for c in cart if c["category"] == cat and remaining[c["item_id"]] > 0]
            ...
            ids = [i.strip() for i in str(d.get("trigger_item_ids", "")).split(",") if i.strip()]
            if not ids:
                continue
            try:
            ...
            if sets > 0:
                normal_unit = sum(
                    next((c["price"] for c in cart if str(c["item_id"]) == i), 0) for i in ids)
...
def get_goal_for_event(event_name):
    goals = get_goals()
    default = {"revenue_goal": 0, "table_cost": 0, "hotel_cost": 0,
               "travel_cost": 0, "other_costs": 0, "notes": ""}
...
def render_goal_progress(event_name, compact=False):
    ...
    if compact:
        ...
        st.markdown(f"**🎯 {event_name}:** ${current:.0f} / ${goal['revenue_goal']:.0f} "
    ...
    if not compact and total_costs > 0:
        ...
        p2.metric("Break-even", f"${total_costs:.2f}")
        ...
        with st.expander("📋 Cost breakdown"):
            if goal["table_cost"] > 0:
                st.write(f"🎪 Table fee: **${goal['table_cost']:.2f}**")
            if goal["hotel_cost"] > 0:
                st.write(f"🏨 Hotel: **${goal['hotel_cost']:.2f}**")
            if goal["travel_cost"] > 0:
                st.write(f"✈️ Travel: **${goal['travel_cost']:.2f}**")
            if goal["other_costs"] > 0:
                st.write(f"🍜 Other: **${goal['other_costs']:.2f}**")
...
if CUSTOMER_MODE:
    ...
    st.caption("Ask about combo deals at checkout! ✨")
    ...
    if sel_ser != "🌐 All":
        items = [i for i in items if i["series"] == sel_ser]
    if search.strip():
    ...
    for i in range(0, len(items), 3):
        ...
        for col, it in zip(cols, items[i:i+3]):
            with col:
                ...
                if it["image_url"]:
                    try:
                        st.image(it["image_url"], width="stretch")
                    except Exception:
                        st.markdown("🎨")
                low = int(it["stock"]) <= 3
...
# ================================================================
    ...
    with tab3:
        ...
        if not goals or not sales:
            ...
            if "event" in df_sales.columns:
                df_sales["event"] = df_sales["event"].astype(str)
            history = []
            for g in goals:
                event = str(g.get("event", "")).strip()
                if event == "_default_" or not event:
                    continue
                ...
                actual = df_sales[df_sales["event"] == event]["total"].sum() if "event" in df_sales.columns else 0
...
# ================================================================
    ...
    if not sales:
        ...
        if "event" in df.columns:
            events = sorted(set(str(e) for e in df["event"] if e))
    ...
    if not sales:
        ...
        if not df_items.empty:
            ...
            if required.issubset(set(df_items.columns)):
                ...
                st.warning(f"⚠️ Sale Items tab missing columns: {', '.join(missing)}")
        ...
        if has_item_data and "item_name" in df_items.columns:
            st.subheader("🏆 Top Sellers")
            top = df_items.groupby("item_name")["qty"].sum().sort_values(ascending=False).head(10)
            st.bar_chart(top)
